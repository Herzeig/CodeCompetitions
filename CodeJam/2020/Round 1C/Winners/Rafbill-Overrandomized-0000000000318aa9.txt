...
void solve(){
  ...
  vector<pair<char, li>> A(begin(M), end(M));
  sort(begin(A), end(A), [&](auto const& a, auto const& b) { return get<1>(a) < get<1>(b); });
  string ans;
  for(auto a : A) ans.push_back(get<0>(a));
  reverse(begin(ans), end(ans));
  for(char c : ans) S.erase(c);
  cout << *S.begin() << ans << endl;
}
