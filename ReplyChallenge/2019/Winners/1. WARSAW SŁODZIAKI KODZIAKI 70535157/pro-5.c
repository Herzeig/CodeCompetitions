...
int main() {
  ...
  for(int rep = 0; rep < 3; ++rep) for(int whom = 0; whom < (int) offices.size(); ++whom) {
    is_office[offices[whom].first][offices[whom].second] = false;
    for(int& x : now_best_distance) {
      x = INF;
    }
    for(int c = 0; c < count_clients; ++c) {
      for(int i = 0; i < (int) offices.size(); ++i) {
        if(i != whom) {
          now_best_distance[c] = min(now_best_distance[c], odl[c][offices[i].first][offices[i].second]);
        }
      }
    }
    pair<int,int> off = find_best_office();
  ...
  vector<vector<int>> his_score(H, vector<int>(W, -INF));
  vector<pair<int,  pair<int,int> >> order;
  for(int row = 0; row < H; ++row) {
    for(int col = 0; col < W; ++col) {
      if(id_client[row][col] == -1 && grid[row][col] != '#') {
        his_score[row][col] = 0;
        for(int c = 0; c < count_clients; ++c) {
          if(odl[c][row][col] == inf) {
            his_score[row][col] += (int) 1e6 + 5;
          }
          else {
            his_score[row][col] += odl[c][row][col];
          }
        }
  ...
  // wypisanie z klientami
  for(int row = 0; row < H; ++row) {
    printf("%s  ", grid[row]);
    for(int col = 0; col < W; ++col) {
      if(id_client[row][col] == -2) {
        grid[row][col] = 'O';
      }
      else if(id_client[row][col] != -1) {
        grid[row][col] = 'C';
      }
    }
    printf("%s\n", grid[row]);
