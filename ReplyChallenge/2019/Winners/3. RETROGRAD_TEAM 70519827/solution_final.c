...
int get_score(int i, int j, int k) {
    ...
    if (ret == -1) return -1e9;
    if (!USE_COSTS) return -ret;
    return CUSTOMERS[k].score - ret;
}
...
map<pair<int, int>, pair<int, vector<pair<int, int>>>> MEM;
pair<int, vector<pair<int, int>>> KMeans(int cc, int k) {
    // if (TEST_IDX == 5 && cc == 0 && (k < 18 || k > 26)) return {-1e9, {}};
    if (MEM.count({cc, k})) return MEM[{cc, k}];
    ...
    vector<vector<int>> rev_assigned(k);

    auto assign = [&]() {
        for (int i = 0; i < k; ++i) rev_assigned[i].clear();
        for (int i = 0; i < (int)custs.size(); ++i) {
            ...
            rev_assigned[assigned[i]].push_back(i);
        }
    };
    ...
    for (int its = 0; its < KMEANS_ITERS; ++its) {
        ...
        auto get_score_at = [&](int i, int j, int k, int best) {
            int ans = POTENCE[i][j];
            for (int c : rev_assigned[k]) {
                int now = get_score(i, j, custs[c]);
                if (now < 0) {
                    ans += now; 
                    // if (ans < best) return ans;
                }
            }
            return ans;
        };
        for (int k = 0; k < (int)cands.size(); ++k) {
            ...
            for (auto itr : positions) {
                int i, j; tie(i, j) = itr;
                int now = get_score_at(i, j, k, best);
                if (now > best) {
                    best = now;
                    ci = i; cj = j;
                }
