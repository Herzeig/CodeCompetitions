...
signed main(){
	...
	vvc<int> vis(h,vi(w,0));
	vc<pi> z;
	rep(i,h)rep(j,w)if(vis[i][j]==0&&board[i][j]!='#'){
		pi cnt;
		queue<pi> q;
		auto reach=[&](int r,int c){
			if(!inc(0,r,h-1)||!inc(0,c,w-1)||vis[r][c])return;
			if(board[r][c]=='#')return;
			if(board[r][c]=='_')cnt.a++;
			else cnt.b++;
			vis[r][c]++;
			q.emplace(r,c);
		};
