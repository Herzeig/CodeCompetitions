...
signed main(){
	...
	
	{
		...
		sort(all(idx),[&](int i,int j){
			return sum[i]*pow(lib[j].t,0.9)>sum[j]*pow(lib[i].t,0.9);
	...
	/*rep(i,ls+1)dp[i].resize(ds);
	
	rep(i,ls){
		rep(j,ds){
			dp[i+1][j]=dp[i][j];
			if(j>=lib[i].t)
				chmax(dp[i+1][j],dp[i][j-lib[i].t]+sum[i]);
		}
	}
	
	cerr<<dp[ls][ds-1]<<endl;
	
	vi idx;
	
	int cur=ds-1;
	per(i,ls){
		if(cur>=lib[i].t&&dp[i+1][cur]==dp[i][cur-lib[i].t]+sum[i]){
			idx.pb(i);
			cur-=lib[i].t;
		}
	}
