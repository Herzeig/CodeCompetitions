...
int calcScore(const vc<string>&board,const vc<Person>&ds,const vc<Person>&ms,const vc<pi>&x,const vc<pi>&y){
	...
	rep(i,M){
		...
		if(r==-1&&c==-1)continue;
		if(!inc(0,r,h-1)||!inc(0,c,w-1)){
			cerr<<"Manager "<<i<<" has invalid position "<<y[i]<<endl;
			exit(1);
		}
		if(board[r][c]!='M'){
			cerr<<"Manager "<<i<<" tried to occupy invalid cell "<<y[i]<<" ("<<board[r][c]<<")"<<endl;
			exit(1);
		}
		if(idx[r][c]!=-1){
			cerr<<"Manager "<<i<<" tried to occupy alraedy occupied cell "<<y[i]<<endl;
			exit(1);
		}
		idx[r][c]=D+i;
	}
